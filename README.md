...
**CelarPass Core** is the underlying cryptographic engine that powers the open-source **CelarPass** desktop application. It provides a suite of tools for generating, analyzing, and protecting credentials with a strong focus on privacy (*offline-first*).
...
You can integrate `celarpass_core` into your own Python projects:

```bash
git clone https://github.com/tu-usuario/celarpass-core.git
cd celarpass-core
...
```python
from celarpass_core.generators import PasswordEngine
...
### 📦 CelarPass (Desktop Application)

If you are looking for an elegant graphical interface, visual vault exports, QR code generation for TOTP, and compliance profiles, discover the desktop application powered by this engine on the official website of **CelarPass PRO**.
...
**CelarPass Core** es el motor criptográfico subyacente que impulsa la aplicación libre de escritorio **CelarPass**. Proporciona una suite de herramientas de generación, análisis y protección de credenciales enfocadas en la privacidad (*offline-first*).
...
Puedes integrar `celarpass_core` en tus propios proyectos Python:
...
### 📦 CelarPass (Aplicación de Escritorio)
