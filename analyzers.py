...
class StrengthAnalyzer:
    ...
    def get_unified_metrics(password: str) -> Tuple[int, str, str, float, str]:
        ...
        if len(password) > 6:
            ...
            if (unique_chars / len(password)) < MIN_UNIQUE_RATIO:
                ...
                warning = QT_TRANSLATE_NOOP("CelarPassApp", "Muchos caracteres repetidos")

        if crack_seconds < 86400:
            val, color, msg = 15, "#e74c3c", QT_TRANSLATE_NOOP("CelarPassApp", "Muy Débil")
        elif crack_seconds < 31536000:
            val, color, msg = 35, "#e67e22", QT_TRANSLATE_NOOP("CelarPassApp", "Débil")
        elif crack_seconds < 315360000:
            val, color, msg = 55, "#f1c40f", QT_TRANSLATE_NOOP("CelarPassApp", "Regular")
        elif crack_seconds < 3153600000:
            val, color, msg = 80, "#2ecc71", QT_TRANSLATE_NOOP("CelarPassApp", "Buena")
        else:
            val, color, msg = 100, "#3498db", QT_TRANSLATE_NOOP("CelarPassApp", "Muy Fuerte")
    ...
    def calculate_entropy_preview(length: int, use_upper: bool, use_lower: bool, use_nums: bool, use_syms: bool) -> Tuple[int, str, str]:
        ...
        if pool_size == 0 or length == 0:
            return 0, "#e74c3c", QT_TRANSLATE_NOOP("CelarPassApp", "Débil")

        entropy = length * math.log2(pool_size)
        val = min(95, int((entropy / 80.0) * 100))
        
        if entropy < 50:
            color, msg = "#e74c3c", QT_TRANSLATE_NOOP("CelarPassApp", "Débil")
        elif entropy < 75:
            color, msg = "#f39c12", QT_TRANSLATE_NOOP("CelarPassApp", "Moderada")
        else:
            color, msg = "#2ecc71", QT_TRANSLATE_NOOP("CelarPassApp", "Fuerte")
