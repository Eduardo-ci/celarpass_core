...
class HIBPClient:
    ...
    def check_password(password: str) -> Tuple[int, str]:
        ...
        if not HAS_REQUESTS:
            return -1, QT_TRANSLATE_NOOP("CelarPassApp", "La librería 'requests' no está instalada.")
        ...
        try:
            ...
            headers = {"User-Agent": "CelarPass Desktop App"}
            ...
            if response.status_code == 200:
                ...
                error_prefix = QT_TRANSLATE_NOOP("CelarPassApp", "Error HTTP")
                return -1, f"{error_prefix} {response.status_code}"
        except Exception as e:
            logging.error(f"Fallo de red HIBP: {e}")
            return -1, QT_TRANSLATE_NOOP("CelarPassApp", "Error de conexión o timeout.")
        finally:
            del password
            del sha1_hash